...
pub fn todo_factory(id: u32, item_type: &str, item_title: &str) -> Result<ItemTypes, &'static str> {
    if item_type.to_lowercase() == "pending" {
        let item = Pending::new(id, item_title);
        Ok(ItemTypes::Pending(item))
    } else if item_type.to_lowercase() == "done" {
        let item = Done::new(id, item_title);
        Ok(ItemTypes::Done(item))
    } else if item_type.to_lowercase() == "todo" {
        let item = Todo::new(id, item_title);
